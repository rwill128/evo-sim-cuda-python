...
class LandCreationTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.smoothed_template = land_creation.add_smoothing_to_template(self.template)
    ...
    def test_entire_surface(self):
        self.entire_surface = land_creation.entire_surface(self.land, self.water)
        self.assertGreaterEqual(self.entire_surface.min(), -0.5)
        self.assertLessEqual(self.entire_surface.max(), 0.5)

        self.assertGreaterEqual(self.entire_surface.max(), 0.0, "Needs at least some positive values")
        self.assertLessEqual(self.entire_surface.min(), 0.0, "Needs at least some negative values")
