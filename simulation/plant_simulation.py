...
def grow_plants(world_params):
    ...
    for index, plant in enumerate(world_params['plants']):
        if plant[0][4] > ENERGY_NEEDED_TO_ADD_BRANCH:
            ...
            new_seg = [1, joined_seg[3], joined_seg[4], joined_seg[3] + np.random.randint(-1, 1),
...
def grow_plants_dictionary_version(world_params):
    for index, plant in enumerate(world_params['plants']):
        if plant['energy'] > 0:
    ...
    for index, plant in enumerate(world_params['plants']):
        if plant['energy'] > ENERGY_NEEDED_TO_ADD_BRANCH:
            ...
            joined_seg = plant['segments'][np.random.randint(0, len(plant['segments'] - 1))]  # TODO: Should only grow off of live segments
            new_seg = [
                1,
                joined_seg[3],
                joined_seg[4],
    ...
    for index, new_plant_segments in new_growth:
        world_params['plants'][index]['segments'] = new_plant_segments
...
def photosynthesize(world_params):
    ...
    for index, x in enumerate(occupied_squares[0]):
        ...
        if world_params['carbon_dioxide_map'][x][y] > 0:
            ...
            world_params['plants'][int(np.round(world_params['world_array'][x][y])) - 1][0][
                4] += ENERGY_GAINED_FROM_ONE_CARBON_DIOXIDE


def photosynthesize_dictionary_version(world_params):
    occupied_squares = np.nonzero(world_params['world_array'])
    for index, x in enumerate(occupied_squares[0]):
        y = occupied_squares[1][index]
        if world_params['carbon_dioxide_map'][x][y] > 0:
            world_params['carbon_dioxide_map'][x][y] -= 1
            world_params['plants'][int(np.round(world_params['world_array'][x][y])) - 1][
