...
def detect_occluded_squares(l: np.array, x_translation: int, y_translation: int, c_id: int, world_array: np.array):
    x0, y0, x1, y1 = l
    world_array[x0 + x_translation, y0 + y_translation] = c_id
    world_array[x1 + x_translation, y1 + y_translation] = c_id
...
def draw_plant(c_id: int, segments: np.array, x_translation: int, y_translation: int, world_array: np.array):
    for l in segments:
        if l[0] > PLANT_SEGMENT_DEAD:
            detect_occluded_squares(l[1:], x_translation, y_translation, c_id, world_array)
